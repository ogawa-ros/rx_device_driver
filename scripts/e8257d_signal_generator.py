...
class e8257d_controller(object):
    def __init__(self):
        ...
        self.pub_freq = rospy.Publisher(
            name="{}_freq".format(name), data_class=Float64, latch=True, queue_size=1
        )
        self.pub_power = rospy.Publisher(
            name="{}_power".format(name), data_class=Float64, latch=True, queue_size=1
        )
        self.pub_onoff = rospy.Publisher(
            name="{}_onoff".format(name), data_class=Int32, latch=True, queue_size=1
        )
